...
# + Integrácia TRIAGE_FOLDERS modulu
...
# TRIAGE FOLDERS – inicializácia
triage = TriageFolders(base_path=".")


class Validator:
    """Wrapper trieda pre validate_request tak, aby COLNÍK mohol volať Validator().validate()."""
...
def validate_request(req):
    ...
    # ============================
    # TRIAGE FOLDERS – kontrola štruktúry
    # ============================
    existing = triage.list_folders()
    required = TRIAGE_FOLDER_RULES["required"]

    for folder in required:
        if folder not in existing:
            errors.append(f"Missing required folder: {folder}")

    # Nepovinné priečinky – len placeholder
    optional = TRIAGE_FOLDER_RULES["optional"]
